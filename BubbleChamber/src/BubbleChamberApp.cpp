...
void BubbleChamberApp::setup()
{
	...
	try {
		...
		mGlowCubeShader = gl::GlslProg(loadAsset("glowCube.vert"), loadAsset("glowCube.frag"));
	}
	...
	mDecalTex = gl::Texture(loadImage(loadAsset("decal.png")));
	mIconTex = gl::Texture(loadImage(loadAsset("iconBubbleChamber.png")), mipFmt);
