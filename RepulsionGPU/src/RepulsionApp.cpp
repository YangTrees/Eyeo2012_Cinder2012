...
void RepulsionApp::setup()
{
	...
	try {
		...
		mPosInitShader	= gl::GlslProg( loadAsset( "passThru.vert" ), loadAsset( "fboPosInit.frag" ) );
		mVelInitShader	= gl::GlslProg( loadAsset( "passThru.vert" ), loadAsset( "fboVelInit.frag" ) );
	...
	// LOAD TEXTURES
	mIconTex		= gl::Texture( loadImage( loadAsset( "iconRepulsion.png" ) ), mipFmt );
