...
{	
	...
	// LOAD SHADERS
	try {
		mBrightStarsShader	= gl::GlslProg( loadAsset( "brightStars.vert" ), loadAsset( "brightStars.frag" ) );
		mFaintStarsShader	= gl::GlslProg( loadAsset( "faintStars.vert" ), loadAsset( "faintStars.frag" ) );
	...
	// LOAD TEXTURES
	mIconTex		= gl::Texture( loadImage( loadAsset( "iconCatalog.png" ) ), mipFmt );
	mMilkyWayTex	= gl::Texture( loadImage( loadAsset( "milkyWay.jpg" ) ) );
	mStarTex		= gl::Texture( loadImage( loadAsset( "star.png" ) ), mipFmt );
	mStarGlowTex	= gl::Texture( loadImage( loadAsset( "starGlow.png" ) ), mipFmt );
	mDarkStarTex	= gl::Texture( loadImage( loadAsset( "darkStar.png" ) ) );
	...
	parseData( getAssetPath( "starData.txt" ) );
...
//			mBrightStarsShader.uniform( "texScale", 0.1f );
	...
	if( mRenderNames ){
		...
		BOOST_FOREACH( Star* &s, mNamedStars ){
			s->drawName( mMousePos, power * mScalePer, max( sqrt( mScalePer ) - 0.1f, 0.0f ) );
		}
	}
